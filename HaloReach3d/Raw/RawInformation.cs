    ...
    public class RawInformation
    {
        ...
        public class RawEntryTable : List<RawEntry>
        {
            public RawEntryTable(HaloMap Map)
            {
                ...
                for (int i = 0; i < Map.Index_Items.Count; i++)
                {
                    ...
                    if (Map.Index_Items[i].Class == "zone")
                    {
                ...
                //Get our zone Offset
                int zoneOffset = Map.Index_Items[zoneIndex].Offset;
                ...
                int rawTablePointer = Map.IO.In.ReadInt32() - Map.Map_Header.mapMagic;
                ...
                for (int i = 0; i < rawTableCount; i++)
                {
                    ...
                    if (rawBlock.TagID != -1)
                        rawBlock.TagName = Map.Index_Items[Map.GetTagIndexByIdent(rawBlock.TagID)].Name;
                    else
                    {
                    ...
                    if (rawBlock.Count0 != 0)
                        rawBlock.Pointer0 = Map.IO.In.ReadInt32() - Map.Map_Header.mapMagic;
                    ...
                    if (rawBlock.Count1 != 0)
                        rawBlock.Pointer1 = Map.IO.In.ReadInt32() - Map.Map_Header.mapMagic;
        ...
        public class ExternalMapsList : List<ExternalMap>
        {
            ...
            public ExternalMapsList(HaloMap Map)
            {
                ...
                int playIndex = -1;
                for (int i = 0; i < Map.Index_Items.Count; i++)
                    if (Map.Index_Items[i].Class == "play")
                    {
                        playIndex = i;
                        break;
                    }
                ...
                if (playIndex != -1)
                {
                    ...
                    HaloMap.TagItem play_tag = Map.Index_Items[playIndex];

                    //Go to the tag offset + 12
                    Map.IO.In.BaseStream.Position = play_tag.Offset + 12;
                    int externCount = Map.IO.In.ReadInt32();
                    int externPointer = Map.IO.In.ReadInt32() - Map.Map_Header.mapMagic;
            ...
            public void CreateIOs()
            {
                for (int x = 0; x < this.Count; x++)
                    if (map.Map_Directory + this[x].MapName != map.Map_Location)
                        this[x].io = new EndianIO(map.Map_Directory + this[x].MapName,
        ...
        public class RawPoolsList : List<RawPool>
        {
            ...
            public RawPoolsList(HaloMap Map)
            {
                ...
                if (playIndex != -1)
                {
                    ...
                    int poolPointer = Map.IO.In.ReadInt32() - Map.Map_Header.mapMagic;
        ...
        public class RawLocationsList : List<RawLocation>
        {
            ...
            public RawLocationsList(HaloMap Map)
            {
                ...
                if (playIndex != -1)
                {
                    ...
                    int locationsPointer = Map.IO.In.ReadInt32() - Map.Map_Header.mapMagic;
